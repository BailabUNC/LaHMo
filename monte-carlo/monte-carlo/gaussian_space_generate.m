...
for i = 1:size
    for j = 1:size
        % epidermis
        voxel_space(i, j, (offset+bump(i, j)+1):(offset+bump(i, j)+l1)) = 1;
        % dermis
        voxel_space(i, j, (offset+bump(i, j)+l1+1):(offset+bump(i, j)+l1+l2)) = 2;
        % adipose
        voxel_space(i, j, (offset+bump(i, j)+l1+l2+1):(offset+bump(i, j)+l1+l2+l3)) = 3;
        % muscle
        voxel_space(i, j, (offset+bump(i, j)+l1+l2+l3+1):(offset+bump(i, j)+l1+l2+l3+l4)) = 4;
    end
end
