...
BLECharacteristic lhmCharacteristic(LHM_CHAR_UUID, BLERead | BLENotify, 200);
...
void sensorInit()
{
    ...
    lsm6dsox.Set_FIFO_Mode(LSM6DSOX_STREAM_MODE); // start batching in continuous mode
...
void serviceInit()
{
    ...
    lhmService.addCharacteristic(lhmCharacteristic);
    BLE.addService(lhmService);

    lhmCharacteristic.setValue("LHM");
}
        ...
        while (central.connected())
        {
            if (canOutput)
            {
                ...
                std::string data = timestamp_str + "," +
                                   ...
                                   pitch_str + "," +
                                   yaw_str;
                lhmCharacteristic.writeValue(data.c_str());
